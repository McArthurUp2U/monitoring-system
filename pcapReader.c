 ...
 * pcapReader.c
 *
 *read pcap file and anylasis procotol it contains.
 */
#include <unp.h>
#include <stdio.h>
#include <stdlib.h>
				       ...
				       struct ndpi_id_struct **dst)
{
  ...
  if ((iph->ihl * 4) > ipsize 
      || (iph->frag_off & htons(0x1FFF)) != 0){//ipsize < ntohs(iph->tot_len)
    return NULL;}
...
static void printResults(u_int64_t tot_usec)
{
  ...
  for (i = 0; i <= ndpi_get_num_supported_protocols(ndpi_struct); i++) {
    if (protocol_counter[i] > 0) {
		sendto(sockfd,&i,sizeof(i),0,(struct sockaddr *)&from,sizeof(from));
...
while (1)
{
	 len = recvfrom(sockfd, &a, sizeof(a), 0,
